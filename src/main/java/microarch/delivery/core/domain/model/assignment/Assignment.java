...
public class Assignment extends BaseEntity<UUID> {
    ...
    public Result<Void, Error> complete(Location courierLocation) {
        ...
        if (this.status == AssignmentStatus.COMPLETED) {
            return Result
                    .failure(GeneralErrors.invalidOperation("Cannot complete assignment that is already completed"));
        }

        int distance = this.location.distanceTo(courierLocation);
        if (distance > 1) {
            return Result.failure(GeneralErrors.invalidOperation(String.format(
                    "Cannot complete assignment: courier is too far from order location. Distance: %d, max allowed: 1",
                    distance)));
        }
